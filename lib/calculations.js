...
// =====================================================
// TIER PRICING GENERATOR (STRICTLY DECREASING)
// =====================================================
...
export function calculateCompleteQuote(quoteData, shopSettings, material) {
  ...
  const { unitPrice, totalPrice } = calculatePricing(
    trueCostPerHat,
    quoteData.target_margin_pct,
    quoteData.rush_pct,
    quoteData.qty,
    quoteData.shipping_charge
  )

  // Calculate tier pricing
  const tierPrices = calculateTierPricing(
    materialCostPerPatch,
    laborCostPerPatch,
    quoteData.tier_quantities || [24, 48, 96, 144, 384, 768],
    quoteData.proof_minutes,
    quoteData.setup_minutes,
    quoteData.packing_minutes,
    quoteData.apply_minutes_per_hat,
...
export function calculatePatchOnlyQuote(quoteData, shopSettings, material) {
  ...
  tierQuantities.forEach(tierQty => {
    ...
    const orderLaborPerUnitT = orderLaborTotal / tierQty
    ...
    const unitPriceTRushRounded = roundToNickel(unitPriceTRush)
    const totalT = roundToDollar(unitPriceTRushRounded * tierQty)
